...
class Main:
    ...
    def connectEvent(self):
        ...
        for index in range(0,len(self.ui.calculate)):
            self.ui.calculate[index].mousePressEvent = lambda event, num = index: self.calculate(event, num)
            self.ui.calculate[index].enterEvent = lambda event, num = index: self.EnterEvent(event, num, 2)
            self.ui.calculate[index].leaveEvent = lambda event, num = index: self.LeaveEvent(event, num, 2)
    ...
    def EnterEvent(self,event,index,judge):
        if judge == 1:
            ...
            self.ui.calculate[index].setStyleSheet(
                "border : 1px solid black;"
                "background-color : grey;"
                )
        elif judge == 3:
